...
class Hestia_Big_Title_Section extends Hestia_First_Front_Page_Section {
	...
	public function get_big_title_content() {
		...
		$button_text_default          = current_user_can( 'edit_theme_options' ) ? esc_html__( 'Returning students', 'hestia' ) : false;
		...
		$button_link_default          = current_user_can( 'edit_theme_options' ) ? esc_url( admin_url( '/login' ) ) : false;
		...
		$secondary_button_text_default          = current_user_can( 'edit_theme_options' ) ? esc_html__( 'Register', 'hestia' ) : false;
		$hestia_big_title_secondary_button_text = get_theme_mod( 'hestia_big_title_secondary_button_text', $secondary_button_text_default );
		if ( ! empty( $hestia_big_title_secondary_button_text ) ) {
			$section_content['secondary_button_text'] = $hestia_big_title_secondary_button_text;
		}

		$secondary_button_link_default          = current_user_can( 'edit_theme_options' ) ? esc_url( admin_url( '/register' ) ) : false;
		$hestia_big_title_secondary_button_link = get_theme_mod( 'hestia_big_title_secondary_button_link', $secondary_button_link_default );
		if ( ! empty( $hestia_big_title_secondary_button_link ) ) {
			$section_content['secondary_button_link'] = $hestia_big_title_secondary_button_link;
		}
	...
	public function show_big_title_content( $content ) {
		...
		">
			...
			<?php } ?>
			<?php if ( ( ! empty( $content['button_link'] ) && ! empty( $content['button_text'] ) ) || ( ! empty( $content['secondary_button_link'] ) && ! empty( $content['secondary_button_text'] ) ) ) { ?>
            <div class="buttons">
				<?php } ?>
				...
				<?php } ?>
				<?php if ( ! empty( $content['secondary_button_link'] ) && ! empty( $content['secondary_button_text'] ) ) { ?>
                    <a href="<?php echo esc_url( $content['secondary_button_link'] ); ?>"
                       title="<?php echo esc_html( $content['secondary_button_text'] ); ?>"
                       class="btn btn-secondary ml-10" <?php echo hestia_is_external_url( $content['secondary_button_link'] ); ?>><?php echo esc_html( $content['secondary_button_text'] ); ?></a>
					<?php hestia_big_title_section_secondary_buttons_trigger(); ?>
				<?php } ?>
				<?php if ( ( ! empty( $content['button_link'] ) && ! empty( $content['button_text'] ) ) || ( ! empty( $content['secondary_button_link'] ) && ! empty( $content['secondary_button_text'] ) ) ) { ?>
            </div>
		<?php } ?>
