...
class Hestia_Big_Title_Controls extends Hestia_Front_Page_Section_Controls_Abstract {
						...
						'slider' => array(
							...
							'hestia_slider_type'      => array(
								'image'    => array(
									...
									'hestia_big_title_secondary_button_text',
									'hestia_big_title_secondary_button_link',
								),
								'parallax' => array(
	...
	/**
	 * Add secondary button controls.
	 */
	public function add_secondary_button_controls() {
		...
		$this->add_control(
			new Hestia_Customizer_Control(
				...
				array(
					'label'    => esc_html__( 'Secondary button text', 'hestia' ),
					'section'  => 'sidebar-widgets-sidebar-big-title',
					'priority' => 40,
				)
			)
		...
		$this->add_control(
			new Hestia_Customizer_Control(
				...
				array(
					'label'    => esc_html__( 'Secondary button URL', 'hestia' ),
					'section'  => 'sidebar-widgets-sidebar-big-title',
					'priority' => 45,
				)
			)
		...
		$this->add_partial(
			new Hestia_Customizer_Partial(
				...
				array(
					'selector'        => '.carousel .buttons',
					'settings'        => 'hestia_big_title_secondary_button_text',
					'render_callback' => array( $this, 'secondary_button_render_callback' ),
				)
			)
		);
	}
	...
	private function maybe_add_defaults_for_big_title() {
		...
		$this->change_customizer_object( 'setting', 'hestia_big_title_title', 'default', esc_html__( 'Where Change Begins', 'hestia' ) );
		$this->change_customizer_object( 'setting', 'hestia_big_title_text', 'default', esc_html__( 'Working with over 80 courts in Oregon, we specialize in changing driver behavior to reduce traffic crashes, injuries and death on Oregon roads.', 'hestia' ) );
		$this->change_customizer_object( 'setting', 'hestia_big_title_button_text', 'default', esc_html__( 'Login', 'hestia' ) );
		$this->change_customizer_object( 'setting', 'hestia_big_title_button_link', 'default', esc_url( '/login' ) );
		$this->change_customizer_object( 'setting', 'hestia_big_title_secondary_button_text', 'default', esc_html__( 'Register', 'hestia' ) );
		$this->change_customizer_object( 'setting', 'hestia_big_title_secondary_button_link', 'default', esc_url( '/register' ) );
	}
	 ...
	 * @return bool|string
	 */
	public function secondary_button_render_callback() {
		$secondary_button_text = get_theme_mod( 'hestia_big_title_secondary_button_text' );
		$secondary_button_link = get_theme_mod( 'hestia_big_title_secondary_button_link' );
		...
		$output = '<a href="' . $secondary_button_link . '" title="' . $secondary_button_text . '" class="btn btn-secondary">' . $secondary_button_text . '</a>';
